...
*Correction, 2026-08-12: that count is now six versus six. An audit found that the scorer decided whether an approval existed but never what it authorized, so one approval could cover unlimited calls. Under the rules that replaced it, an approval names a tool and carries a budget, and the score records one entry per gated **call** rather than per gate. The demo's property is unchanged and was re-verified: byte-identical outputs, differing only in authorization. Only the number moved, and it moved because the old number was counting the wrong thing.*
