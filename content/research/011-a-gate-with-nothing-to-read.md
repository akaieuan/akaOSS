...
It is worth being precise about which part of Jev is new, because the answer decides what a buyer is paying for and what a gate can rely on.

TypeSafe says it built a new model architecture, a parallel sampler and the RLCD training method, and describes none of the three beyond those names. Within a week of the launch, an open project called [OpenJev](https://github.com/kvnloo/openjev) reproduced the interface with open components: it takes a state and a set of declared options, runs one forward pass of a 4-billion-parameter open-weight model, and reads the probability of each option straight off the logits, with no decoding, no JSON and no repair step, on a single consumer GPU. Its README says plainly that it reproduces the interface pattern and not the undisclosed model or training.

That is the honest boundary. Typed output with a distribution attached is something any language model can be made to give you, because the last layer already holds a distribution over the next token, and restricting the answer to a fixed option set is a matter of reading it rather than sampling from it. The shape guarantee, the speed and the free output all follow from that reading. What does not follow is calibration. A raw logit distribution is the model's habit, not its accuracy, and turning it into a number that means what it says is the part TypeSafe claims to have trained for and the part nobody outside can inspect.

So the proprietary claim and the unverifiable claim are the same claim. For a gate this is clarifying rather than discouraging: `confidenceGate` does not care where its number comes from, and neither does the check. Whichever source fills the field, Jev, an open logit reader, or something not yet built, the question is identical, whether its 0.85 was right about 85 times in a hundred on your cases, and the only instrument that answers it is the labelled trace.
