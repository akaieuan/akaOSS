...
The confidence itself is a statistic of the model's own distribution. In the interactive explorer on their confidence page, a Choice confidence is computed from the top probability and the number of options alone, which means a split of 60, 40 and 0 reports the same 0.40 as a split of 60, 20 and 20, though the first is a two-way contest and the second is not. They say plainly that this is a default and that the full distribution is returned so you can compute something else, and the production calculation may differ from the documentation demo. The general point survives either way: this is the model describing its own uncertainty, and [№ 002](/research/002-an-instrument-not-a-judge) is about why an instrument should not be asked to vouch for itself.
...
Nothing ships from this note. Three things are now worth doing that were not before.
...
Every figure quoted is TypeSafe's: the latency range, the pricing, the multiples, the zero. Their evaluation references are other models, the workflows are their own, the training data is undisclosed, and the model is text-only at version 1.13. Our description of how confidence is computed comes from a documentation demo and may not match production.
