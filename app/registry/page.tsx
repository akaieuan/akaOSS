...
function InstallRow({ name, title, description, type, registryDependencies }: (typeof REGISTRY_ITEMS)[number]) {
  ...
  return (
    <div className="group settle grid grid-cols-1 gap-4 border-t border-border/60 py-8 sm:grid-cols-[240px_1fr]">
        ...
        {registryDependencies && registryDependencies.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {registryDependencies.map((d) => (
              <span
                key={d}
                className="rounded-full border border-border/40 bg-background/40 px-2 py-0.5 font-mono text-meta text-muted-foreground"
              >
                {d}
              </span>
            ))}
        ...
        <section className="pb-16">
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            <div className="flex items-start gap-2 rounded-xl border border-border/40 bg-card/40 px-3 py-3">
              <pre className="flex-1 overflow-x-auto whitespace-pre font-mono text-xs leading-relaxed text-foreground">
                {TOKENS}
              </pre>
